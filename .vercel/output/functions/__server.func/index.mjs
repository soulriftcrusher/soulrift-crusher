...
function renderWebManifest(hostHeader) {
	...
	const name = fromHost === "Grok App" ? "Soulrift Crusher" : fromHost;
	return JSON.stringify({
		name,
		short_name: name.length > 12 ? "Soulrift" : name,
		description: "Idle dungeon RPG. Hire heroes, crush floors, craft gear, fight in the arena, and war across servers.",
		id: "/",
		start_url: "/",
		scope: "/",
		display: "standalone",
...
async function grokPwaMiddleware(event, next) {
	...
	if (path === "/__grok/manifest.webmanifest" || path === "/__grok/manifest.json" || path === "/manifest.webmanifest" || path === "/manifest.json") return new Response(renderWebManifest(requestHost(event)), { headers: {
