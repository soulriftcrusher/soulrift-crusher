import { DISCORD_INVITE } from "@/game/discord";
import { cn } from "@/lib/utils";

function Clyde({ className }: { className?: string }) {
  return (
...
export function DiscordBtn({ className }: { className?: string }) {
  return (
    <a
      href={DISCORD_INVITE}
      target="_blank"
      ...
      className={cn(
        "flex h-12 w-full items-center justify-center gap-2 rounded-md font-display text-sm tracking-wide text-white",
        className,
      )}
      style={{ background: "#5865F2" }}
    >
      <Clyde className="size-5" />
      Join Discord
    </a>
  );
}
