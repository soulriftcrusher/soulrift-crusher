...
const SITE_HOSTS: string[] = ["soulriftcrusher.com", "www.soulriftcrusher.com"];
const SITE_ORIGINS: string[] = SITE_HOSTS.map((h) => `https://${h}`);
const baseURL = explicitBaseURL ?? {
  ...
  fallback: "https://soulriftcrusher.com",
};
...
export const auth = betterAuth({
  ...
  plugins: [
    ...
    // Bridges Better Auth's Set-Cookie into TanStack Start when the helper exists.
    // MUST be last. Safe if setCookie is missing on this request (published hunt).
    safeStartCookies(),
  ],
});
