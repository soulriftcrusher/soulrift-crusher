      ...
      <head>
        ...
        <script
          dangerouslySetInnerHTML={{
            __html: `if(location.hostname==='soulriftcrusher.com')location.replace('https://www.soulriftcrusher.com'+location.pathname+location.search+location.hash);(function(){var meta=document.querySelector('meta[name=viewport]');var locked='width=device-width, initial-scale=1, minimum-scale=1, maximum-scale=1, user-scalable=no, viewport-fit=cover';var last=1;function snap(){var vv=window.visualViewport;var s=vv&&vv.scale?vv.scale:1;if(Math.abs(s-last)<0.05)return;last=s;if(meta)meta.setAttribute('content',locked);document.documentElement.style.zoom=s>1.05?String(1/s):'';}document.addEventListener('touchstart',function(e){if(e.touches&&e.touches.length>1)e.preventDefault();},{passive:false});document.addEventListener('touchmove',function(e){if(e.touches&&e.touches.length>1)e.preventDefault();},{passive:false});document.addEventListener('gesturestart',function(e){e.preventDefault();});document.addEventListener('gesturechange',function(e){e.preventDefault();});document.addEventListener('wheel',function(e){if(e.ctrlKey)e.preventDefault();},{passive:false});if(window.visualViewport)visualViewport.addEventListener('resize',snap);})();if('serviceWorker'in navigator)navigator.serviceWorker.register('/sw.js?v=9',{scope:'/'});window.addEventListener('error',function(e){var m=String(e&&e.message||'');if(m.indexOf('removeChild')<0&&m.indexOf('NotFoundError')<0)return;try{if(sessionStorage.getItem('soulrift.recover')==='1')return;sessionStorage.setItem('soulrift.recover','1');}catch(x){}location.reload();});try{if(sessionStorage.getItem('soulrift.recover')==='1')sessionStorage.removeItem('soulrift.recover');}catch(x){}`,
          }}
        />
        <script
